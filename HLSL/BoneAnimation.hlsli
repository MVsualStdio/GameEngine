...
struct VertexIn
{
    ...
    float4 color : COLOR;
    int4 boneIndex : TEXCOORD1;
    float4 widget : TEXCOORD2;
};
...
VertexOut VS(VertexIn vIn)
{
    ...
    float4 localPosition = float4(vIn.pos,1.0);

    if(vIn.boneIndex[0] >= 0){
        localPosition = mul(float4(vIn.pos,1.0),boneMatrices[vIn.boneIndex[0]]);
    } 

    float4 posW = mul(localPosition, g_World);
